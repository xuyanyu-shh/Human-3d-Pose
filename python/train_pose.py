...
parser.add_argument('--start-epoch', default=0, dest='start_epoch', type=int, metavar='N',
                    help='manual epoch number (useful on restarts)')
parser.add_argument('-b', '--batch-size', default=256, dest='batch_size', type=int,
...
parser.add_argument('--print-freq', '-p', default=10, dest='print_freq', type=int,
...
parser.add_argument('-nFeats', type = int, 
                   default = 256, help = '# features in the hourglass')
parser.add_argument('-nStack', type = int, 
                   default = 2, help = '# hourglasses to stack')
parser.add_argument('-nModules', type = int, 
                    default = 2, help = '# residual modules at each hourglass')
parser.add_argument('-nRegModules', type = int, 
                    default = 2, help = '#depth regression modules')
parser.add_argument('-DEBUG', type = int, 
                    default = 0, help = 'DEBUG level')
parser.add_argument('-regWeight', type = float, 
...
def train(epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        Acc.update(Accuracy((output[args.nStack - 1].data).cpu().numpy(), (target2D_var.data).cpu().numpy()))
        mpjpe, num3D = MPJPE((output[args.nStack - 1].data).cpu().numpy(), (reg.data).cpu().numpy(), meta)
        if num3D > 0:
